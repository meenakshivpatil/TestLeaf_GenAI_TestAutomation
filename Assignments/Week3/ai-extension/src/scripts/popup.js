...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    const awsBdrkApiKeyInput = document.getElementById('awsBdrkApiKey');
    ...
    const modelsByProvider = {
      ...
      testleaf: [
        { value: 'testleaf-1', label: 'Testleaf 1' },
        { value: 'testleaf-2', label: 'Testleaf 2' },

      ],
      TingTing: [
        { value: 'TingTing-1', label: 'TingTing 1' },
        { value: 'TingTing-2', label: 'TingTing 2' },

      ],
      'aws-bdrk': [
        { value: 'openai.gpt-oss-20b-1:0', label: 'openai.gpt-oss-20b-1:0' },
        { value: 'openai.gpt-oss-120b-1:0', label: 'openai.gpt-oss-120b-1:0' },
    ...
    if (awsBdrkApiKeyInput) {
      awsBdrkApiKeyInput.addEventListener('change', (e) => {
        storage.set({ awsBdrkApiKey: e.target.value });
      });
    }
    ...
    function updateApiKeyVisibility(selectedModel) {
      ...
      const awsBdrkKeyContainer = document.getElementById('awsBdrkKeyContainer');
      ...
      if (providerSelect.value === 'groq') {
        ...
        awsBdrkKeyContainer.style.display = 'none';
        ...
        awsBdrkKeyContainer.style.display = 'none';
      } else if (providerSelect.value === 'aws-bdrk') {
        groqContainer.style.display = 'none';
        openaiContainer.style.display = 'none';
        testleafContainer.style.display = 'none';
        awsBdrkKeyContainer.style.display = 'block';
      } else  {
        groqContainer.style.display = 'none';
        openaiContainer.style.display = 'none';
        testleafContainer.style.display = 'block';
    ...
    // Load saved values
    const result = await new Promise(resolve => {
      storage.get(['groqApiKey', 'openaiApiKey','testleafApiKey','awsBdrkApiKey', 'selectedModel', 'selectedProvider', 'githubToken', 'githubRepo', 'githubBranch'], resolve);
    });
